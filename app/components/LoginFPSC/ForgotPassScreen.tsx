...
const ForgotPassScreen: React.FC<ForgotPassScreenProps> = ({ show, onClick }) => {
  ...
  return (
    <div>
      ...
      <div
        className={`fixed top-1/2 left-1/2 w-1/2 h-1/2 bg-slate-200 shadow-lg rounded-2xl z-20 transform transition-transform duration-700 ease-in-out flex items-center justify-center`}
        style={{ transform: show ? 'translateX(-50%) translateY(-50%)' : 'translateX(-250%) translateY(-50%)' }}
      >
        <div className='relative flex flex-col justify-around items-center w-[95%] h-[90%] bg-white'>
          <RxCrossCircled
            onClick={handleCrossClick}
            className='absolute top-1 right-1 text-black hover:text-red-500 cursor-pointer'
            size={40}
          />
